...
if (isset($_POST['id']) && isset($_POST['idtable']) && !empty($_POST['id']) && !empty($_POST['idtable'])){
	...
	if (($_SESSION['id']==$retour['Joueur1']) OR ($_SESSION['id']==$retour['Joueur2'])){
	...
	else if ($retour['actif'] == 0){//vérification, parce que nous ne savons pas si quelqu'un à deja cliquer et de l'autre côté la partie était encore proposé.
		...
		header("Location: ./../panel.php"); //si l'utilisateur à ecrit nimps.
	}
?>